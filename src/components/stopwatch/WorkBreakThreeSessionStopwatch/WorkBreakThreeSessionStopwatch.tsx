...
const WorkBreakThreeSessionStopwatch: React.FC<IProps> = () => {
  const { breakThreeDuration, updateBreakThreeSession } = useSessionTracker();
  ...
  const { seconds, minutes, hours, days, isRunning, start, pause } =
    useStopwatch({
      ...
      offsetTimestamp: dayjs()
        .add(breakThreeDuration[0], "d")
        .add(breakThreeDuration[1], "h")
        .add(breakThreeDuration[2], "m")
        .add(breakThreeDuration[3], "s")
        .toDate(),
    });

  React.useEffect(() => {
    updateBreakThreeSession(days, hours, minutes, seconds);
